...
func main() {
	...
	if len(c.SourceEs) > 0 {
		...
		for slice:=0;slice<c.ScrollSliceSize ;slice++  {
			scroll, err := migrator.SourceESAPI.NewScroll(c.SourceIndexNames, c.SourceTypeNames, c.ScrollTime, c.DocBufferCount, c.Query,slice,c.ScrollSliceSize, c.Fields)
			if err != nil {
				log.Error(err)
				return
			}
	...
	if len(c.TargetEs) > 0 {
		...
		if len(c.SourceEs) > 0 {
			...
			indexNames, indexCount, sourceIndexMappings, err := migrator.SourceESAPI.GetIndexMappings(c.CopyAllIndexes, c.SourceIndexNames, c.SourceTypeNames)
			if err != nil {
				log.Error(err)
				return
			}
